...
class COINSingle(IM_Base):
    ...
    def run(self):
        ...
        for r in np.arange(1, self.rounds+1):
            ...
            print("Round %d" % (r))
            self.get_context()
            under_explored = self.under_explored_nodes(r)
            ...
            # Otherwise, run TIM
                ...
                print("TIM")
                self.dump_graph(self.inf_ests, ("tim_"+self.graph_file))
            ...
            self.counters += tried_cnts
            self.successes += success_cnts
            for edge_idx, cnt in enumerate(self.counters):
                self.inf_ests[edge_idx] = self.successes[edge_idx] / cnt if(cnt > 0) else (0)
    ...
    def under_explored_nodes(self, round_idx):
        ...
        if(len(under_exp_nodes) > self.seed_size):
            ...
            self.dump_graph(self.inf_ests, ("undertim_"+self.graph_file))
