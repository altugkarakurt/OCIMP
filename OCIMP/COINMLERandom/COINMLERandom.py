...
class COINMLERandom(IM_Base):
    def __init__(self, seed_size, graph_file, rounds, context_dims=2, 
                gamma=0.4, epsilon=0.1, alpha=1, beta=19):
        """------------------------------------------------------------
        ...
        self.inf_ests = np.array([[(random() * 0.1) for idx, _ in enumerate(self.edges[:,0])] for _ in range(self.context_cnt)])
    ...
    def run(self):
        ...
        for r in np.arange(1, self.rounds+1):
            ...
            for edge_idx, cnt in enumerate(self.counters[context_idx]):
                self.inf_ests[context_idx][edge_idx] = ((self.alpha + self.successes[context_idx][edge_idx])) / (cnt + self.beta + self.alpha) \
                                                        if(cnt > 0) else (random() * 0.1)
            fail_cnts = tried_cnts - success_cnts
            self.update_globals(context_idx, tried_cnts, success_cnts, fail_cnts)
    ...
    def update_globals(self, context_idx, tried_cnts, success_cnts, fail_cnts, eta=0.000001):
        upper_bound = 500
        lower_bound = 0
        ...
        while(True):
            ...
            error = sum([1/(beta_est + self.counters[context_idx][edge_idx] - self.successes[context_idx][edge_idx]) for edge_idx in fail_idxs]) - \
                    sum([1/(self.alpha + self.successes[context_idx][edge_idx]) for edge_idx in success_idxs])
